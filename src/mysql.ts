...
export default async (client: Client) => {
  ...
  const createData = async (table: string, body: any) => {
    ...
    try {
      await db.con.query(
        `INSERT INTO ${table} (${bodykeys.join(", ")}) VALUES (${VALUES})`
      );
  ...
  const delData = async (table: string, where: any) => {
    ...
    const WHERE = wherekeys
      .map((k) => {
        return `${k} = ${where[k] === null ? "NULL" : `'${where[k]}'`}`;
      })
      .join(" AND ");
    try {
      ...
      await channel.send({ content: `<@&839158574138523689> ${err}` });
      console.error(
        `Error deleting ${table} where ${JSON.stringify(where)}: ${err}`
      );
    }
  };
  ...
  const setData = async (table: string, where: any, body: any) => {
    ...
    const SET = bodykeys
      .map((k) => {
        return `${k} = ${body[k] === null ? "NULL" : `'${body[k]}'`}`;
      })
      .join(", ");
    const data = await db.con.query(`SELECT * FROM ${table} WHERE ${WHERE}`);
    console.log(
      `Set ${table} where ${JSON.stringify(where)} to ${JSON.stringify(body)}`
    );
