    ...
    <section class="flex mx-auto max-w-6xl px-6 items-center" style="min-height: calc(100vh - 64px);">
      <div class="text-center sm:text-left justify-start" style="filter: drop-shadow(0 5rem 10rem rgba(79, 70, 229, 1));">
        <div class="flex sm:hidden relative justify-center align-center sm:ml-auto mb-10" style="width: 100%;">
          ...
          <img class="rounded-full z-10 animate-float ease-in-out" src={`/assets/images/${(client.user ? client.user.username : 'Cactie').replace(/ /g, '')}.webp`} style="max-width: 40%" alt={client.user ? client.user.username : 'Cactie'} />
        </div>
        <h1 class="font-bold tracking-tight text-white text-5xl">
          The last <span class="text-blue-400">Discord</span> bot you need, <span class="text-green-200">{client.user ? client.user.username : 'Cactie'}</span>.
        </h1>
      ...
      <div class="hidden sm:flex relative justify-end align-center sm:ml-auto" style="max-width: 50%;">
        ...
        <img class="rounded-full z-10 animate-float ease-in-out" src={`/assets/images/${(client.user ? client.user.username : 'Cactie').replace(/ /g, '')}.webp`} alt={client.user ? client.user.username : 'Cactie'} />
      </div>
    </section>
  );
});
