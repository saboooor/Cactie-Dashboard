...
export default component$(({ store, lighter, label, centered, onSwitch$ }: any) => {
  ...
  return (
    ...
    }} class={`flex items-center group transition ease-in-out text-black/50 ${lighter ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-800 hover:bg-gray-700'} rounded-lg px-3 py-2 ${loadingStore.loading ? `${store.dev === undefined ? 'opacity-0' : 'opacity-50'} pointer-events-none` : ''} ${centered ? 'm-auto' : ''}`}>
